...
module.exports = {
	...
	execute(message, args, client) {
		...
		if (!member) {
			member = memberObj.create(memberUserID);
				...
				Member.findOne({ userID: victim.id })
					.then((victimDB) => {
						...
						const multiplier = parseFloat(
							(Math.random() * (stealConfig.highestReturn - stealConfig.lowestReturn) +
								stealConfig.lowestReturn).toFixed(2)
						); //Multiplier between 0.2 - 0.5
						...
						if (roll < 0.5) {
							...
							message.channel.send(
								`${message.member.user.username} failed to steal from ${victim.user.username}`
							);
							member.stealTime = stealConfig.TIME;
							return;
						}
